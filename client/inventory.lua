...
function vRP.giveInventoryItemToVehicle(user_id, idname, amount, notify)
  if string.sub(idname, 1, 6) == "trunk:" then
    local vehicle_id = string.sub(idname, 7)
    ...
    if notify then
      vRPclient.notify(user_id, {"Added "..amount.." "..idname.." to the trunk."})
    end
  else
    -- Regular inventory management logic
  end
end
