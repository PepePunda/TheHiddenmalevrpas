...
function tvRP.getNearbyOwnedVehicles(radius)
	...
	local ret = {}
	local ok = false
	print("[Vehicle Debug] Checking nearby vehicles within radius: " .. radius) -- ADDED DEBUG
	for k,v in pairs(vehicles) do
		-- Ensure v[3] is a valid entity handle before getting coords/checking driveable status
		if type(v[3]) == "number" and DoesEntityExist(v[3]) then
			local x,y,z = table.unpack(GetEntityCoords(v[3],true))
			local dist = #(vec3(x, y, z) - vec3(px, py, pz))
			local driveable = IsVehicleDriveable(v[3])
			print("[Vehicle Debug] Found vehicle: " .. v[2] .. " (Type: "..k.."), Dist: " .. dist .. ", Driveable: " .. tostring(driveable)) -- ADDED DEBUG
			if dist <= radius+0.0001 and driveable then
				print("[Vehicle Debug]   -> Adding vehicle to list.") -- ADDED DEBUG
				table.insert(ret, {v[1],v[2],GetVehicleVisualName(v[2]),dist})
				ok = true
			-- else -- Optional: Add reason for not adding
				-- print("[Vehicle Debug]   -> Not adding. Reason: Dist="..dist..", Driveable="..tostring(driveable))
			end
		else
			print("[Vehicle Debug] Found invalid vehicle reference in vehicles table for type: " .. k) -- ADDED DEBUG
		end
	end
	return ok, ret
end
