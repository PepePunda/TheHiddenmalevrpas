...
function vRP.transformerStep(name, player, player_source)
    ...
    if user_id and def and def.items_in and def.items_out then
        ...
        if all_items then
            -- take all input items
            for k, v in pairs(def.items_in) do
                vRP.tryGetInventoryItem(user_id, k, v, true)
            end

            -- determine where to place the output items
            local in_trunk = false -- this should be dynamically determined based on the menu state
            if in_trunk then
                local nearest_vehicle = vRP.getNearestOwnedVehiclePlate(player)
                if nearest_vehicle then
                    for k, v in pairs(def.items_out) do
                        vRP.giveInventoryItemToVehicle(user_id, "trunk:"..nearest_vehicle, k, v, true)
                    end
                end
            else
                for k, v in pairs(def.items_out) do
