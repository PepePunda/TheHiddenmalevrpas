...
window.addEventListener("load",function(){
  ...
  dynamic_menu.onReturn = function(){ $.post("http://vrp/menu",JSON.stringify({act: "return", id: dynamic_menu.id})); }
 ...
 $.post("http://vrp/cfg","");
  ...
  window.addEventListener("message",function(evt){ //lua actions
    ...
    if(data.act == "cfg"){
      ...
      if(css.top)
        dynamic_menu.div.style.top = "350px";
      if(css.right)
        dynamic_menu.div.style.right= css.right;
      if(css.header_color)
    ...
    // PROMPT
      ...
      wprompt.open(data.title, data.text, data.numberOnly);
    }
    // REQUEST
